///////////////////////////////////////////////////////////////////////////////
//  Filename: $RCSfile: CharacteristicPolynomialCoefficients.java,v $
...
public class CharacteristicPolynomialCoefficients implements Feature
{
        ...
        Map properties) throws FeatureException
    {
        ...
        if (atomLabelName != null)
        {
            ...
            {
                logger.error("Property '" + atomLabelName +
        ...
        if (bondLabelName != null)
        {
            ...
            {
                logger.error("Property '" + bondLabelName +
                    "' must be an bond type to calculate the " + getName() +
                    ".");
        ...
        Matrix weightedAdjacency = new Matrix(mol.getAtomsSize(),
                mol.getAtomsSize());
        Atom atom;
        ...
        for (int i = 1; i <= mol.getAtomsSize(); i++)
        {
            ...
            if (atomLabelName != null)
            {
                weightedAdjacency.set(atom.getIndex() - 1, atom.getIndex() - 1,
                    atomProperties.getDoubleValue(atom.getIndex()));
            }
            else
            {
                weightedAdjacency.set(atom.getIndex() - 1, atom.getIndex() - 1,
        ...
        for (int i = 0; i < mol.getBondsSize(); i++)
        {
            ...
            if (bondLabelName != null)
            {
                weightedAdjacency.set(bond.getBeginIndex() - 1,
                    bond.getEndIndex() - 1,
                    bondProperties.getDoubleValue(bond.getIndex()));
                weightedAdjacency.set(bond.getEndIndex() - 1,
                    bond.getBeginIndex() - 1,
                    bondProperties.getDoubleValue(bond.getIndex()));
            }
            else
            {
                weightedAdjacency.set(bond.getEndIndex() - 1,
                    bond.getBeginIndex() - 1, 1.0);
                weightedAdjacency.set(bond.getBeginIndex() - 1,
                    bond.getEndIndex() - 1, 1.0);
            }
        }

        Matrix A_n = (Matrix) weightedAdjacency.clone();

        double[] coefficients = (double[]) DynamicArrayResult.getNewArray(
        ...
        for (int i = 0; i < mol.getAtomsSize(); i++)
        {
            ...
            Matrix identity = Matrix.identity(mol.getAtomsSize(),
                    mol.getAtomsSize(), a_n);
