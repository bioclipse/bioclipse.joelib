///////////////////////////////////////////////////////////////////////////////
//  Filename: $RCSfile: MolSparseInstance.java,v $
...
//                           2003,2004,2005
//  Copyright (c) University of Waikato, Australia
//
...
public class MolSparseInstance extends MolInstance
{
     ...
     */
    public MolSparseInstance(double weight, double[] attValues, int[] indices,
        int maxNumValues)
    {
        int vals = 0;
    ...
    public static void main(String[] options)
    {
        try
        {
            ...
            System.out.println("Length of copy missing: " +
                MolInstance.isMissingValue(copy.value(length)));
            System.out.println("Missing value coded as: " +
                MolInstance.missingValue());
    ...
    public MolInstance mergeInstance(MolInstance inst)
    {
        ...
        return new MolSparseInstance(1.0, values, indices,
                ...
                else
                {
                            ...
                            m_Dataset.attribute(m_Indices[i]).isString())
                    {
                        try
                        {
                            text.append(m_Indices[i] + " " +
                                Utils.quote(
                                    m_Dataset.attribute(m_Indices[i]).value(
                                        (int) valueSparse(i))));
                        }
                        catch (Exception e)
                        {
                            e.printStackTrace();
                            System.err.println(new MolInstances(
                                    (MolInstances) m_Dataset, 0));
